...
<form class="box" action="index_li.php" method="post">
  ...
  <h5 style="color: #2ecc71;" >
    ...
    {
      echo $_SESSION['msg'];//show notification messages for user
    } 
    if (isset($_SESSION['log_out_flag']) && $_SESSION['log_out_flag'])//if log out show message
      {
        $log_out="User logged out successfully";
        echo $log_out;
        $_SESSION['log_out_flag']= false;
        unset($_SESSION['log_out_flag']);
        session_destroy();
      }
    ?> 
  </h5>
  ...
  {window.location.replace("http://localhost/web_project/index.php");
  ...
  {window.location.replace("http://localhost/web_project/index_li.php");
